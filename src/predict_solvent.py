...
def main() -> None:
    ...
    radius = args.radius if args.radius is not None else float(checkpoint.get("radius", 8.0))
    dmin = float(checkpoint.get("dmin", 0.0))
    step = float(checkpoint.get("step", 0.2))

    graph = build_crystal_graph(args.cif, radius=radius, max_neighbors=args.max_neighbors)
    structure = read_cif_structure(args.cif)
    pos = structure.frac_coords.astype(np.float32)
    tensors = tuple(tensor.to(device) for tensor in graph_to_tensors(graph, pos, radius, dmin, step))
    ...
    with args.output_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(["atom_index", "atom_label", "element", "fract_x", "fract_y", "fract_z", "probability", "pred_label"])
