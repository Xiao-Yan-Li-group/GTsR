...
from model.cif_utils import subset_pymatgen_structure, write_labeled_cif, write_pymatgen_cif
from predict_solvent import predict_solvent_atoms, write_prediction_csv
...
def component_formula_rows(solvent_structure) -> list[dict]:
    ...
    try:
        ...
        from pymatgen.analysis.local_env import JmolNN  # type: ignore
...
def main() -> None:
    ...
    stem = args.cif.stem
    removed_cif = args.output_dir / f"{stem}_removed.cif"
    solvent_cif = args.output_dir / f"{stem}_solvent.cif"
    predictions_csv = args.output_dir / f"{stem}_predictions.csv"
    summary_csv = args.output_dir / f"{stem}_removed_summary.csv"
    marked_cif = args.output_dir / f"{stem}_marked.cif"
    ...
    write_prediction_csv(predictions_csv, structure, probabilities, labels, include_kept=True)
